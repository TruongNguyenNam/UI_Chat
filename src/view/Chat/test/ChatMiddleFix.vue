<template>
  ...
  // Hàm scroll xuống cuối chat - dùng nextTick để đảm bảo DOM đã render xong
  ...
  watch(
    ...
    async (friend) => {
      ...
      const currentChatId = friend.chatId; // Guard để tránh race condition
  
      loading.value = true;
      try {
        const res = await MessageService.getChatMessages(friend.chatId);
  
        // Nếu trong lúc await mà user đã chuyển sang chat khác → bỏ qua
        if (props.selectedFriend?.chatId !== currentChatId) return;
        ...
        if (props.selectedFriend?.chatId === currentChatId) {
          loading.value = false;
        }
      }
    },
  ...
  // Chuẩn hóa tin nhắn từ WebSocket
  ...
  const subscribeToChat = (chatId: number) => {
    ...
    currentSubId.value = subscribe(topic, async (stompMsg) => {
      const body = JSON.parse(stompMsg.body);
      const incoming = normalizeIncomingMessage(body);
  
      // Nếu là tin của mình → cố gắng thay thế optimistic message
      if (incoming.senderId === currentUserId) {
        // Tìm optimistic message (có id âm hoặc isPending = true)
        const optimisticIndex = messages.value.findIndex(
          (m) => (m.id < 0 || m.isPending)
        );
  
        if (optimisticIndex !== -1) {
          // Thay thế bằng tin thật từ server
          messages.value[optimisticIndex] = { ...incoming, isPending: false, isError: false };
          await scrollToBottom();
          return;
        }
      }
  
      // Nếu không tìm thấy pending → kiểm tra xem có trùng content + thời gian gần không (phòng trường hợp miss)
      const duplicateIndex = messages.value.findIndex((m) => 
        m.content === incoming.content &&
        m.messageType === incoming.messageType &&
        Math.abs(new Date(m.sentAt).getTime() - new Date(incoming.sentAt).getTime()) < 5000 // trong 5 giây
      );
  
      if (duplicateIndex !== -1) {
        messages.value[duplicateIndex] = incoming;
        await scrollToBottom();
        return;
      }
  
      // Tin mới hoàn toàn → push vào cuối
      messages.value.push(incoming);
      await scrollToBottom();
    });
  };
  
  // Các hàm còn lại giữ nguyên (retry, file, reply, reaction, format time...)
...
<!-- tôi đang gặp vấn đề là mỗi lần khi chọn vào đoạn chat thì thanh cuộn trong tin nhắn cứ phải kéo xuống và nhắn tin xong rồi gửi đi, nếu nhắn tiếp sẽ phải click lại vào ipunt -->